...
""")
        ...
        for target in targets:
            ...
            if os.path.exists(t_path):
                ...
                # Make slstatus.sh executable if we are in slstatus dir
                if target == "slstatus":
                    sl_sh = os.path.join(t_path, "slstatus.sh")
                    if os.path.exists(sl_sh):
                        os.system(f"chmod +x {sl_sh}")
                        console.print(f"      [green]✔[/green] [dim]Made slstatus.sh executable[/dim]")
